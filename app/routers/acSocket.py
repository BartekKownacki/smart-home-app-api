...
from models.ACSocketStateModel import crud, schemas, models
import dependencies
...
async def get_last_ac_state(deviceId:int, db: Session = Depends(dependencies.get_db), current_user_id: int = Depends(dependencies.get_current_user_id)):
    state = await crud.get_last_ac_state(db, deviceId, current_user_id)
    return state

@router.post("/createState/{deviceId}", response_model=schemas.AcSocket)
async def create_new_ac_state(deviceId: int, acState: schemas.AcSocketCreate, db: Session = Depends(dependencies.get_db), current_user_id: int = Depends(dependencies.get_current_user_id)):
    return await crud.create_ac_state(db=db, acState=acState, user_id=current_user_id, deviceId = deviceId)