...
def get_all_users(skip: int = 0, limit: int = 100, db: Session = Depends(dependencies.get_db), current_user: schemas.User = Depends(dependencies.get_current_active_user)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/info")
async def get_actual_user_info(current_user: schemas.User = Depends(dependencies.get_current_active_user)):
    return current_user


@router.get("/token/refresh")
async def refresh_token(current_user: schemas.User = Depends(dependencies.get_current_active_user)):
    result = dependencies.create_access_token(
