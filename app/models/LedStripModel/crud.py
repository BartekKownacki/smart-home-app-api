...
def get_last_led_stripe_state(db: Session, deviceId: int, user_id: int):
    ...
    if not db_obj:
        return schemas.LedStripBase(state = False,
                                        color_red = 0,
                                        color_green = 0,
                                        color_blue = 0)

    state_to_return = schemas.LedStripBase(state = db_obj.state,
...
def create_led_stripe_state(db: Session, ledStripState: schemas.LedStripCreate, user_id: int, deviceId: int):
